		...
		function SectionFunctions:Tab(Settings)
			...
			function TabFunctions:Section(Settings)
                ...
                function SectionFunctions:Slider(Settings, Flag)
                    ...
                    local Value = Settings.Default or Settings.Minimum
                    ...
                    sliderDisplay.FontFace = Font.new(assets.interFont)
                    sliderDisplay.Text = tostring(Value)
                    sliderDisplay.TextColor3 = Color3.fromRGB(255, 255, 255)
                    sliderDisplay.TextSize = 13
                    sliderDisplay.TextTransparency = 0.5
                    sliderDisplay.TextXAlignment = Enum.TextXAlignment.Right
                    sliderDisplay.AutomaticSize = Enum.AutomaticSize.Y
                    sliderDisplay.BackgroundTransparency = 1
                    sliderDisplay.Position = UDim2.fromScale(1, 0)
                    sliderDisplay.Size = UDim2.fromScale(0, 0)
                    ...
                    local function UpdateValue(value)
                        ...
                        if Settings.Precision == 0 then
                            value = math.round(value)
                        else
                            value = math.floor(value * (10 ^ Settings.Precision)) / (10 ^ Settings.Precision)
                        end
                
                        local displayMethod = Settings.DisplayMethod or "Value"
                        local displayValue = value
                        if displayMethod == "Percent" then
                            displayValue = math.floor((value - Settings.Minimum) / (Settings.Maximum - Settings.Minimum) * 100)
                            sliderDisplay.Text = displayValue .. "%"
                        else
                            sliderDisplay.Text = tostring(displayValue)
                        end
                
                        local percent = (value - Settings.Minimum) / (Settings.Maximum - Settings.Minimum)
                        sliderHead.Position = UDim2.fromScale(percent, 0.5)
                        ...
                        if Settings.OnValueChanged then
                            task.spawn(function()
                                Settings.OnValueChanged(value) 
                            end)
                        end
                    end
                    ...
                    local function BeginSlide(input)
                        ...
                        local value = Settings.Minimum + (Settings.Maximum - Settings.Minimum) * percent
                    ...
                    local function EndSlide()
                        Sliding = false
                        if Settings.OnSlideEnd then
                            task.spawn(function()
                                Settings.OnSlideEnd(SliderFunctions.Value)
                            end)
                        end
                    end
                    ...
                    function SliderFunctions:UpdateValue(value, skipCallback)
                        ...
                        local displayMethod = Settings.DisplayMethod or "Value"
                        if displayMethod == "Percent" then
                            local displayValue = math.floor(percent * 100)
                            sliderDisplay.Text = displayValue .. "%"
                        else
                            sliderDisplay.Text = tostring(value)
                        end
                
                        if not skipCallback and Settings.Callback then
                            task.spawn(function()
                                Settings.Callback(value)
                            end)
                        end
